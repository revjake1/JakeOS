...
- [x] 4.5  **Cowork input** — bottom input field that posts to a Cowork session (or queues for one) for new todos / ideas / questions. _Shipped in `jakeos-web` via `POST /cowork` (Hono route in `src/server.js`) using a deterministic v1 classifier: text ending in `?` routes to `POST /ideas`, otherwise to `POST /todos` with `source: "dashboard"`. Spec scenario "must never silently drop the input" satisfied. Richer Cowork-classifier path can land as a follow-up._
